...
def upload():
    if request.method == 'POST':
        ...
        if not os.path.exists(os.path.join(workDir,fname[:2])):
            os.mkdir(os.path.join(workDir,fname[:2]))
        file.save(os.path.join(workDir,fname[:2],fname))    #Наконец сохраняем файл на сервер
...
def dowloand(name):
    try:
        return send_from_directory(os.path.join(workDir,name[:2]),name,as_attachment=True)
    except:
...
def delete(name):
    try:
        os.remove(os.path.join(workDir,name[:2],name))
        return 'FILE REMOVED'
    except:
        return 'NO SUCH FILE'